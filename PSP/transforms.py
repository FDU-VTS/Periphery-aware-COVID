...
class RandomCrop(object):
    ...
    def __call__(self, img, mask):
        ...
        if self.padding > 0:
            ...
            mask1 = np.zeros((img.shape[0]+pad, img.shape[1]+pad, img.shape[2]+pad))

            bg = int(self.padding/2)
            img1[bg:bg+img.shape[0], bg:bg+img.shape[1], bg:bg+img.shape[2]] = img
            mask1[bg:bg+img.shape[0], bg:bg+img.shape[1], bg:bg+img.shape[2]] = mask
        ...
        return img[x1:x1+tw, y1:y1+th, z1:z1+td], mask[x1:x1+tw, y1:y1+th, z1:z1+td]
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img, mask):
        ...
        if random.random() < 0.5:
            return np.array(img[:, :, ::-1]), np.array(mask[:, :, ::-1]) #.transpose(Image.FLIP_LEFT_RIGHT)
        return img, mask
...
class RandomYFlip(object):
    ...
    def __call__(self, img, mask):
        ...
        """
        if random.random() < 0.5:
            return np.array(img[:, ::-1, :]), np.array(mask[:, ::-1, :])
        return img, mask
