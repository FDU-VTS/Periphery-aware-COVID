...
from dataset1 import Lung2D_ccii_patient
...
parser.add_argument('--n_classes', '-n_cls', default=3, type=int, help='n_classes')
...
def main():
    ...
    if not args.distpre:
        # target_model = resnet50(pretrained=False,num_classes=args.n_classes) 
    ...
    train_data = Lung2D_ccii_patient(train=True,n_classes=args.n_classes)
    val_data = Lung2D_ccii_patient(train=False,n_classes=args.n_classes)
...
def val(net, val_loader, epoch, vis):
    ...
    print('val epoch:', epoch, ' val acc: ', acc, 'recall:', recall, "precision:", precision)
    vis.plot('val acc', acc)
    vis.log('epoch:{epoch},val_acc:{val_acc},val_cm:{val_cm},recall:{recall},precision:{precision}'.format(epoch=epoch,val_acc=acc,val_cm=str(con_matx.value()),recall=recall,precision=precision))   
